# RL World Drop 
# Description: all objects value of +10, 1 or 2 categories of blobs
# George Kachergis & Greg Cox, Sep 14, 2011
...
def get_block_stimuli(num_cats, valence, totExemps=8):
	# valence is now interleaved/blocked for 4-exemplar 2-category blocks,
	# or small_size/large_size for 6-/2-exemplar 2-category blocks
	targets = []
	foils = []
	half = totExemps/2
	small_size = 2
	large_size = totExemps-small_size 
	if num_cats==0: # 8, really..
		for i in range(half):
			blobs = make_blobs.makeCategories(1,[2])[0]
			targets.append(blobs[0])
			foils.append(blobs[1])
		for i in range(half):
	...
	elif num_cats==2 and valence=="interleaved": # was pos in v1
		cats = make_blobs.makeCategories(2,[totExemps,totExemps])
		targets = [ cats[0][0] , cats[1][0] , cats[0][1] , cats[1][1] , cats[0][2] , cats[1][2] , cats[0][3] , cats[1][3] ]
		unique = make_blobs.makeCategories(4,[1,1,1,1])
		foils = [cats[0][4] , cats[1][4]] + unique[0] + unique[1] + [cats[0][5] , cats[1][5]] + unique[2] + unique[3]
		foil_types = ["similar"]*2 + ["unique"]*2 + ["similar"]*2 + ["unique"]*2
	elif num_cats==2 and valence=="blocked": # was pos in v1
		cats = make_blobs.makeCategories(2,[totExemps,totExemps])
		targets = cats[0][:half] + cats[1][:half]
		unique = make_blobs.makeCategories(4,[1,1,1,1])
		foils = cats[0][half:half+2] + unique[0] + unique[1] + cats[1][half:half+2] + unique[2] + unique[3]
		foil_types = ["similar"]*2 + ["unique"]*2 + ["similar"]*2 + ["unique"]*2
	elif num_cats==2 and valence=="diff_sizeSim": # small cat foils are similar to cat
		cats = make_blobs.makeCategories(2,[totExemps,small_size+4])
		targets = cats[0][:large_size] + cats[1][:small_size]
		unique = make_blobs.makeCategories(2,[1,1])
		foils = cats[0][large_size:large_size+2] + cats[1][small_size:small_size+2] + unique[0] + unique[1] + cats[1][small_size+2:small_size+4]
		foil_types = ["similar"]*2 + ["other"]*2 + ["unique"]*2 + ["similarSm"]*2
	elif num_cats==2 and valence=="diff_sizeOth": # small cat foils are from other cat
		cats = make_blobs.makeCategories(2,[totExemps+2,small_size+2])
		targets = cats[0][:large_size] + cats[1][:small_size]
		unique = make_blobs.makeCategories(2,[1,1])
		foils = cats[0][large_size:large_size+2] + cats[1][small_size:small_size+2] + unique[0] + unique[1] + cats[0][large_size+2:large_size+4]
		foil_types = ["similar"]*2 + ["other"]*2 + ["unique"]*2 + ["otherSm"]*2
	elif num_cats==2 and valence=="diff_sizeUni": # small cat foils are unique
		cats = make_blobs.makeCategories(2,[totExemps,small_size+2])
		targets = cats[0][:large_size] + cats[1][:small_size]
		unique = make_blobs.makeCategories(4,[1,1,1,1])
		foils = cats[0][large_size:large_size+2] + cats[1][small_size:small_size+2] + unique[0] + unique[1] + unique[2] + unique[3]
		foil_types = ["similar"]*2 + ["other"]*2 + ["unique"]*2 + ["uniqueSm"]*2
...
def create_blocks():
	blocks = []
	pos = [10]
	for i in range(2): # of no structure (all pos)
	...
	for i in range(2): # 8 2 cat, same valence -> 3 same / 3 diff / 2 novel -- 2 interleaved study
		targets, foils, foil_types = get_block_stimuli(2, "interleaved")
		blocks.append(Block(2, "interleaved", targets, foils, foil_types, pos*8))
	for i in range(2): # 8 2 cat, same valence -> 3 same / 3 diff / 2 novel -- 2 blocked study
		targets, foils, foil_types = get_block_stimuli(2, "blocked")
		blocks.append(Block(2, "blocked", targets, foils, foil_types, pos*8))
	for i in range(4): # 2 cats: 6 and 2 -> 2 same, 2 diff / 2 same, 2 diff
		targets, foils, foil_types = get_block_stimuli(2, "diff_sizeSim")
		vals = pos*8
		blocks.append(Block(2, "diff_size", targets, foils, foil_types, vals))
	for i in range(4): # 2 cats: 6 and 2 -> 2 same, 2 diff / 2 same, 2 diff
		targets, foils, foil_types = get_block_stimuli(2, "diff_sizeOth")
		vals = pos*8
		blocks.append(Block(2, "diff_size", targets, foils, foil_types, vals))
	for i in range(4): # 2 cats: 6 and 2 -> 2 same, 2 diff / 2 same, 2 diff
		targets, foils, foil_types = get_block_stimuli(2, "diff_sizeUni")
		vals = pos*8
		blocks.append(Block(2, "diff_size", targets, foils, foil_types, vals))
	bl1 = blocks[0]
	blocks = blocks[1:]
...
class Block:
	def __init__(self, ncats, valence, targets, foils, foil_types, vals, reps_per_obj=1, study_time=4000, feedback=1, study_type='seq'):
		self.targets = []
		self.ncats = ncats
		self.valence = valence # interleaved / blocked / 
...
def main():
	...
	for bl in range(len(blocks)):
		...
		shuff_study = (blocks[bl].valence=="diff_size") # shuffle diff_size category blocks
		disp_seq_study(blocks[bl].targets, study_time=study_time, reps=1, shuffle=shuff_study)
